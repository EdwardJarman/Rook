...
export type Bot = { id: string; name: string; role: string; purpose: string; avatar: string; color: string; icon: string; status: "Ready" | "Working" | "Paused"; memory: string; approvalRule: string; model: string; lastActive: string };
...
export function useWorkroom() { const context = useContext(WorkroomContext); if (!context) throw new Error("useWorkroom must be used within WorkroomProvider"); return context; }
