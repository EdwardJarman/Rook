...
export const appRouter = router({
  ...
  nodes: router({
    ...
    linkedFolders: router({
      /**
       * Returns the recent folders the user has linked on any of their
       * devices. The desktop window uses this to populate the workspace
       * picker after a fresh sign-in. The actual file reads happen on the
       * user's own computer (Rook Node / desktop), never here.
       */
      list: protectedProcedure.query(({ ctx }) =>
        Promise.resolve(linkedFolderStore.listFor(ctx.user.id))
      ),
      add: protectedProcedure
        ...
        .mutation(({ ctx, input }) => {
          const entry = linkedFolderStore.add(ctx.user.id, input.name, input.path);
          return { folder: entry };
        }),
      remove: protectedProcedure
        .input(z.object({ path: z.string().min(1).max(1024) }))
        .mutation(({ ctx, input }) => {
          const removed = linkedFolderStore.remove(ctx.user.id, input.path);
          return { removed };
        }),
    }),
  }),
...
/**
 * Per-user store of the folders the user has linked through the desktop
 * app. Held in-memory because:
 *   - the canonical store is the user's own Tauri `plugin-store` JSON
 *     (see rook-node/src/app/lib/store.ts), and
 *   - the cloud copy is a "recent folders" list for picker suggestions
 *     on a new device. A user-scoped in-memory map is sufficient for that
 *     and avoids the cross-device persistence hardening that isn't needed
 *     for a recent-folders list.
 *
 * A future InstantDB entity can replace this when we want durable
 * per-user cross-device sync.
 */
class LinkedFolderStore {
  private readonly byUser = new Map<string, LinkedFolderEntry[]>();

  listFor(userId: string): LinkedFolderEntry[] {
    return [...(this.byUser.get(userId) ?? [])]
      .sort((a, b) => b.lastUsedAt.localeCompare(a.lastUsedAt))
      .slice(0, 20);
  }

  add(userId: string, name: string, path: string): LinkedFolderEntry {
    const now = new Date().toISOString();
    const existing = (this.byUser.get(userId) ?? []).find(
      (entry) => entry.path === path,
    );
    if (existing) {
      const next: LinkedFolderEntry = {
        ...existing,
        name,
        lastUsedAt: now,
      };
      this.byUser.set(
        userId,
        (this.byUser.get(userId) ?? []).map((entry) =>
          entry.path === path ? next : entry,
        ),
      );
      return next;
    }
    const entry: LinkedFolderEntry = {
      id: `lf-${Math.random().toString(36).slice(2, 10)}`,
      userId,
      name,
      path,
      addedAt: now,
    ...
    this.byUser.set(userId, [entry, ...(this.byUser.get(userId) ?? [])]);
    return entry;
  }

  remove(userId: string, path: string): boolean {
...
const linkedFolderStore = new LinkedFolderStore();
