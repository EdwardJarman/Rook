...
export const appRouter = router({
  ...
  workroom: router({
    reply: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const devices = notificationEnabled
          ? await db.getPushDevicesForUser(ctx.user.id)
          : [];
        const deliveries = await Promise.all(
          devices
            .filter((device) =>
              needsApproval ? device.approvalEnabled : device.completionEnabled,
            )
            .map((device) =>
              sendExpoPushAlert({
        ...
        return {
          ...
          pushDelivery: {
            ...
            recipients: deliveries.filter((delivery) => delivery.accepted)
              .length,
          },
        };
      }),
  ...
  voice: router({
    transcribe: protectedProcedure
      ...
      .mutation(async ({ input }) => ({
        text: await transcribeOpenRouterAudio(input),
      })),
  }),
  github: router({
    status: protectedProcedure.query(({ ctx }) =>
      githubConnectionStatus(ctx.user.id),
    ),
    authorizationUrl: protectedProcedure
      .input(z.object({ returnTo: z.string().url().max(500).optional() }))
      .mutation(({ ctx, input }) =>
        createGithubAuthorizationUrl(ctx.user.id, input.returnTo),
      ),
    disconnect: protectedProcedure.mutation(({ ctx }) =>
      deleteGithubConnectionForUser(ctx.user.id).then(() => ({ ok: true })),
    ),
    repos: protectedProcedure
      .input(z.object({ search: z.string().max(120).optional() }).optional())
      .query(({ ctx, input }) => listGithubRepos(ctx.user.id, input?.search)),
    selectRepo: protectedProcedure
      .input(z.object({ fullName: z.string().min(3).max(140) }))
      .mutation(({ ctx, input }) =>
        selectGithubRepo(ctx.user.id, input.fullName),
      ),
    unselectRepo: protectedProcedure
      .input(z.object({ fullName: z.string().min(3).max(140) }))
      .mutation(async ({ ctx, input }) => {
        await unselectGithubRepo(ctx.user.id, input.fullName);
        return { ok: true };
      }),
  }),
  excel: router({
    status: protectedProcedure.query(({ ctx }) =>
      microsoftConnectionStatus(ctx.user.id),
    ),
    authorizationUrl: protectedProcedure
      .input(z.object({ returnTo: z.string().url().max(500).optional() }))
      .mutation(({ ctx, input }) =>
        createMicrosoftAuthorizationUrl(ctx.user.id, input.returnTo),
      ),
    disconnect: protectedProcedure.mutation(({ ctx }) =>
      db.deleteMicrosoftConnection(ctx.user.id),
    ),
    disconnectAccount: protectedProcedure
      .input(z.object({ accountId: z.string().min(1).max(80) }))
      .mutation(({ ctx, input }) =>
        disconnectMicrosoftAccount(ctx.user.id, input.accountId),
      ),
    ...
    workbooks: protectedProcedure
      .input(
        z
          .object({ accountId: z.string().min(1).max(80).optional() })
          .optional(),
      )
      .query(({ ctx, input }) =>
        listExcelWorkbooks(ctx.user.id, input?.accountId),
      ),
  ...
  notifications: router({
    register: protectedProcedure
      ...
      .mutation(({ ctx, input }) =>
        db
          .upsertPushDevice(ctx.user.id, input)
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const deliveries = await Promise.all(
          devices
            .filter((device) =>
              input.kind === "approval"
                ? device.approvalEnabled
                : device.completionEnabled,
            )
            .map((device) =>
              sendExpoPushAlert({
                expoPushToken: device.expoPushToken,
  ...
  records: router({
    list: protectedProcedure.query(({ ctx }) =>
      db.listNormalizedWorkroomRecords(ctx.user.id),
    ),
  }),
  preferences: router({
    get: protectedProcedure.query(async ({ ctx }) => {
      ...
      return {
        approval: stored?.approvalEnabled ?? true,
        completion: stored?.completionEnabled ?? true,
      };
    }),
    save: protectedProcedure
      .input(z.object({ approval: z.boolean(), completion: z.boolean() }))
      .mutation(({ ctx, input }) =>
        db.upsertNotificationPreferences(ctx.user.id, {
  ...
  accountData: router({
    export: protectedProcedure.query(({ ctx }) =>
      db.exportAccountWorkroomData(ctx.user.id),
    ),
  ...
  nodes: router({
    list: protectedProcedure.query(({ ctx }) =>
      db.listRookNodesForUser(ctx.user.id),
    ),
    ...
    issueDesktopPairingCode: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        const issued = await db.issueDesktopPairingCode(
          ctx.user.id,
          input.requestId.toLowerCase(),
        );
        if (!issued)
          throw new Error(
            "This desktop connection has expired or is unavailable. Start again from Rook Node.",
          );
        return {
          code: issued.code,
    ...
    rename: protectedProcedure
      ...
      .mutation(({ ctx, input }) =>
        db.renameRookNode(ctx.user.id, input.nodeId, input.name),
      ),
    remove: protectedProcedure
      ...
      .mutation(({ ctx, input }) =>
        db.revokeRookNode(ctx.user.id, input.nodeId),
      ),
    commands: protectedProcedure.query(({ ctx }) =>
      db.listRecentNodeCommands(ctx.user.id),
    ),
        ...
        db
          ...
          .then((record) => ({
            decided: Boolean(
              ...
              (record?.state === "declined" && input.decision === "declined"),
            ),
          })),
      ),
    cloud: router({
...
async function notifyNodeApprovalRequest(
  userId: string,
  summary: string,
): Promise<void> {
  try {
