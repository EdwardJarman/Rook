      ...
      >
        ...
        <View style={{ gap: 12 }}>
          <SectionHeader
            ...
            caption="Give your Bots useful access without giving up control."
          />
          <ExcelConnectionCard />
...
function ProfileAndStorage({ syncStatus }: { syncStatus: string }) {
  ...
  const exportQuery = trpc.accountData.export.useQuery(undefined, {
    enabled: false,
  });
    ...
    try {
      ...
      if (Platform.OS === "web") {
        ...
        await Sharing.shareAsync(uri, {
          ...
          dialogTitle: "Export Rook account data",
        });
      }
    } catch {
      Alert.alert(
        "Export unavailable",
        "Rook could not prepare your account export right now. Please try again.",
      );
    } finally {
      setExporting(false);
    }
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: () =>
            Alert.alert(
              "Final confirmation",
              "This cannot be undone. Delete your cloud workroom now?",
              [
                    ...
                    void deleteMutation
                      ...
                      .then((deleted) => {
                        if (deleted) {
                          ...
                          Alert.alert(
                            "Workroom deleted",
                            "Your account is now an empty Rook workroom.",
                          );
                        }
                      })
                      .catch(() =>
                        Alert.alert(
                          ...
                          "Rook could not delete your data right now. Please try again.",
                        ),
                      );
                  },
                },
  ...
  const signOut = () => {
    ...
    Alert.alert(
      "Sign out of Rook?",
      "Your cloud workroom stays safely saved to your account.",
      [
        { text: "Cancel", style: "cancel" },
        {
          text: "Sign out",
  ...
  return (
    <View style={{ gap: 18 }}>
      <SectionHeader
        title="Profile and storage"
        caption="Everything about your session in one predictable place."
      />
      ...
      <Card>
        <View style={{ flexDirection: "row", alignItems: "center", gap: 13 }}>
          ...
          <View style={{ flex: 1, minWidth: 0 }}>
            ...
            <Text
              numberOfLines={1}
              style={{ color: colors.textSoft, fontSize: 12.5, marginTop: 3 }}
            >
        ...
        <View style={{ height: 1, backgroundColor: colors.line }} />
        <StorageRow
          title="Cloud workroom"
          detail="Managed database per authenticated account"
        >
          <StatusPill
            label={syncStatus}
            tone={
        ...
        <StorageRow
          ...
          detail={
            records.data
              ? `${records.data.bots.length} Bots · ${records.data.tasks.length} tasks · ${records.data.files.length} files`
              : "Loading independently queryable records"
          }
        >
  ...
  return (
      ...
      style={({ pressed }) => [
        {
          ...
          borderWidth: 1,
          borderColor:
            tone === "danger" ? tint(colors.coral, 0.3) : colors.line,
          borderRadius: 18,
    ...
    >
      <MaterialIcons
        name={icon}
        size={19}
        color={tone === "danger" ? colors.coral : colors.textSoft}
      />
      <View style={{ flex: 1 }}>
        <Text style={{ color: foreground, fontSize: 14, fontWeight: "600" }}>
          {label}
        </Text>
        <Text
          style={{
