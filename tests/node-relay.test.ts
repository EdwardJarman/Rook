...
describe("desktop one-time codes", () => {
  it("generates opaque request ids and readable grouped codes", () => {
    ...
    expect(normalizeDesktopPairingCode(code.toLowerCase())).toBe(
      code.replace("-", ""),
    );
  ...
  it("builds the web handoff without a localhost callback or secret", () => {
    ...
    expect(
      buildDesktopPairingUrl({
        serverUrl: "https://www.rook.lighting/",
        requestId,
      }),
    ).toBe(`https://www.rook.lighting/connect-node?request=${requestId}`);
  });

  it("preserves the pairing request through browser hydration and sign-in", () => {
    const connectScreen = readFileSync(
      resolve(process.cwd(), "app/connect-node.tsx"),
      "utf8",
    );
    ...
    expect(connectScreen).toContain(
  ...
  it("validates only a normalized well-formed desktop completion body", () => {
    ...
    ).toMatchObject({ requestId, code: "ABCDEFGH" });
    expect(
      validateDesktopPairingComplete({
        requestId,
        code: "bad",
...
describe("uplink auth header", () => {
  it("parses nodeId:secret pairs and rejects malformed ones", () => {
    expect(parseUplinkAuth("Bearer node-1:rks-x")).toEqual({
      nodeId: "node-1",
      secret: "rks-x",
    });
...
describe("command envelope builder", () => {
  it("produces a version-1 envelope with cloud device binding and deadline", () => {
    ...
    expect((envelope.deadline as number) - (envelope.issuedAt as number)).toBe(
      60_000,
    );
...
describe("payload validation", () => {
  ...
  it("rejects malformed sync bodies", () => {
    ...
    expect(
      validateSyncRequest({ version: "1", results: "nope" }),
    ).toBeUndefined();
    expect(
      validateSyncRequest({
        version: "1",
        results: [{ commandId: 5, ok: true }],
      }),
    ).toBeUndefined();
  });
...
describe("browser pairing links", () => {
  it("builds a connect URL carrying state and loopback port", () => {
    const url = buildConnectNodeUrl({
      serverUrl: "https://www.rook.lighting/",
      state: "s".repeat(48),
      port: 37831,
    });
    expect(url).toBe(
      "https://www.rook.lighting/connect-node?state=" +
        "s".repeat(48) +
        "&port=37831",
    );
  });
  ...
  it("builds a loopback callback URL with encoded params", () => {
    const url = buildPairCallbackUrl({
      port: 37831,
      token: "rkp-a b",
      state: "s".repeat(48),
    });
    expect(
      url.startsWith("http://127.0.0.1:37831/pair?token=rkp-a%20b&state="),
    ).toBe(true);
  });

  it("parses only well-formed callbacks", () => {
    const good = parsePairCallback(
      new URLSearchParams({ token: "rkp-abc123", state: "s".repeat(48) }),
    );
    expect(good?.token).toBe("rkp-abc123");
    expect(
      parsePairCallback(
        new URLSearchParams({ token: "rkp-abc123", state: "short" }),
      ),
    ).toBeUndefined();
    expect(
      parsePairCallback(new URLSearchParams({ state: "s".repeat(48) })),
    ).toBeUndefined();
    expect(
      parsePairCallback(
        new URLSearchParams({ token: "garbage", state: "s".repeat(48) }),
      ),
    ).toBeUndefined();
    expect(
      parsePairCallback(
        new URLSearchParams({
          token: "rkp-" + "x".repeat(200),
          state: "s".repeat(48),
        }),
      ),
    ).toBeUndefined();
