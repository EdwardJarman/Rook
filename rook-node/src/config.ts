...
export const ROOK_NODE_VERSION = "0.1.0";
...
export function defaultConfig(overrides: Partial<RookConfig> = {}): RookConfig {
  const dataHome = overrides.dataHome ?? path.join(rookDataHomeDefault(), "Rook Node");
...
function rookDataHomeDefault(): string {
  ...
  if (platform === "win32") return path.join(process.env.APPDATA ?? os.homedir(), "Rook");
  return path.join(os.homedir(), ".local", "share");
}

/** Resolves a workspace subdirectory, refusing to escape the root. */
export function workspaceDir(config: RookConfig, key: keyof typeof WORKSPACE_LAYOUT): string {
...
export function ensureWorkspace(config: RookConfig): void {
  for (const key of Object.keys(WORKSPACE_LAYOUT) as (keyof typeof WORKSPACE_LAYOUT)[]) {
...
function fsMkdir(dir: string): void {
  try {
    ...
    throw new Error(`Rook Node could not create ${dir}: ${(error as Error).message}`);
  }
}
